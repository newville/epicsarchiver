...
"""
    ...
    def __init__(self, dbconn=None,  **kw):
        ...
        self.fig = Figure((6.0,4.0),dpi=96)
        self.axis= self.fig.add_axes([0.12,0.12,0.80,0.80], axisbg='#FFFFFF')
        ...
        self.labelfont.set_size(11)
        self.titlefont.set_size(12)


    def format_date(self,x,p=None):
	...
	epv1.connect()
        ...
        if n_dat == 1:
            ...
            for d in dat:
                tdat.append(d[0])
                ydat.append(d[1])
            self.axis.step(tdat,ydat)
            
            img = StringIO()
            self.canvas.print_png(img)
            img.seek(0)
            self.write('<img src="data:image/png,%s"><br>' % urllib.quote(img.getvalue()))
                       
#             f = open(f_png,'w')
#             for i in img.read(): f.write(i)
#             f.close()
